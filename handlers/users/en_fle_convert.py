...
@dp.callback_query_handler(lambda c: c.data == "en_pdf_to_pptx")
async def convert_pdf_to_pptx_handler(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "Submit your PDF file! 📄")
...
async def handle_pdf_to_pptx(message: types.Message):
    if message.document.mime_type == "application/pdf":
        ...
        await message.reply_document(types.InputFile(pptx_path), caption="The PDF file has been converted to PPTX! 📊")
        os.remove(file_path)
        os.remove(pptx_path)
...
@dp.callback_query_handler(lambda c: c.data == "en_jpg_to_png")
async def convert_jpg_to_png_handler(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "Submit your JPG file! 🖼️")
    await bot.answer_callback_query(callback_query.id)
...
async def handle_jpg_to_png(message: types.Message):
    ...
    await message.reply_document(types.InputFile(png_path), caption="JPG file converted to PNG! 📷")
    os.remove(file_path)
    os.remove(png_path)
...
@dp.callback_query_handler(lambda c: c.data == "en_json_csv")
async def convert_json_to_csv_handler(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "Submit your JSON file! 📄")
    await bot.answer_callback_query(callback_query.id)
...
async def handle_json_to_csv(message: types.Message):
    if message.document.mime_type == "application/json":
        ...
        await message.reply_document(types.InputFile(csv_path), caption="JSON file converted to CSV! 📈")
        os.remove(file_path)
        os.remove(csv_path)
...
@dp.callback_query_handler(lambda c: c.data == "en_zip")
async def zip_prompt(callback_query: types.CallbackQuery):
    await bot.send_message(callback_query.from_user.id, "Submit your files to create a ZIP! 📦")
    await bot.answer_callback_query(callback_query.id)
...
async def handle_document_zip(message: types.Message):
    ...
    try:
        ...
        await message.reply("The file has been added to the ZIP archive. Submit /finish_zip to extract the archive. 📦")
...
async def finish_zip(message: types.Message):
    ...
    if os.path.exists(zip_path):
        await message.reply_document(types.InputFile(zip_path), caption="ZIP archive is ready! 📦")
        os.remove(zip_path)  # ZIPni foydalanuvchiga yuborganingizdan keyin o'chirish
    else:
        await message.reply("The ZIP archive has not been created yet. Submit file and add to ZIP! 📦")
